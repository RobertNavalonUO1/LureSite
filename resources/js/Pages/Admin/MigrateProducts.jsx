...
// Returns a de-duplicated, trimmed list of candidate image URLs for a product
...
// Read CSRF token from page meta for fetch requests
const getCsrfToken = () => {
    if (typeof document === "undefined") return "";
    return document.querySelector('meta[name="csrf-token"]')?.content || "";
};

// Parse textarea input (lines or commas) into non-empty URL list
...
export default function MigrateProducts() {
    ...
    const migrateProductRequest = async (productId) => {
        ...
        try {
            ...
            // Keep generic error when backend doesn't return JSON
        }
    ...
    const buildPrimaryOverrides = useCallback((productIds) => {
        const overrides = {};
        productIds.forEach((id) => {
            const primaryUrl = primaryImages[id];
            if (primaryUrl) overrides[id] = primaryUrl;
        });
        return overrides;
    }, [primaryImages]);
