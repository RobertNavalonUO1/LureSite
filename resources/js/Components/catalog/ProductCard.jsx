    ...
    <article className="group mx-auto flex h-full w-full max-w-sm flex-col overflow-hidden rounded-3xl border border-amber-100 bg-white/95 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl focus-within:ring-2 focus-within:ring-amber-200">
      <div className="relative">
        <button
          ...
          className="absolute right-3 top-3 z-20 flex h-9 w-9 items-center justify-center rounded-full bg-white/95 text-slate-600 shadow-lg transition hover:scale-110 hover:text-rose-500"
        ...
        {(product.badge || product.is_new || product.fast_shipping) && (
          <span className="absolute left-3 top-3 z-20 rounded-full bg-amber-600 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-white shadow">
            {badgeLabel}
          </span>
        )}
      ...
      <div className="flex flex-grow flex-col gap-2.5 p-4">
        <div className="space-y-1">
          ...
          <h3 className="min-h-[2.75rem] line-clamp-2 text-[15px] font-semibold leading-5 text-slate-900">{title}</h3>
        </div>
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
            <p className="font-medium text-lime-700">{shippingLabel}</p>
            <span className="font-medium text-slate-500">{stockStatus}</span>
          </div>
        </div>
        ...
        <div className="mt-auto grid grid-cols-2 gap-2">
          <button
            ...
            className={`flex min-w-0 items-center justify-center gap-1.5 rounded-full px-3 py-2 text-xs font-semibold transition ${
              product.stock === 0
          ...
          <button
            type="button"
            onClick={goToProduct}
            className="flex min-w-0 items-center justify-center gap-1.5 rounded-full border border-amber-100 px-3 py-2 text-xs font-semibold text-amber-700 transition hover:border-amber-200 hover:bg-amber-50"
          >
