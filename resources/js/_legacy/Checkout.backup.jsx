...
export default function Checkout() {
  ...
  const handleAddressSaved = () => { setShowAddressModal(false); setSelectedAddress(null); setModalMode('create'); router.reload({ only: ['addresses','defaultAddressId'], preserveScroll: true }); };
  const [showAddressModal, setShowAddressModal] = useState(false);
  ...
  const setAsDefault = (addrId) => {
    const putTo = (typeof route === 'function') ? route('addresses.default', addrId) : /addresses//default;
    router.put(putTo, {}, { preserveScroll: true, onSuccess: () => router.reload({ only: ['addresses','defaultAddressId'], preserveScroll: true }) });
  };
  const deleteAddress = (addrId) => {
    if (!confirm('¿Eliminar esta dirección?')) return;
    const delTo = (typeof route === 'function') ? route('addresses.destroy', addrId) : /addresses/;
    router.delete(delTo, { preserveScroll: true, onSuccess: () => router.reload({ only: ['addresses','defaultAddressId'], preserveScroll: true }) });
  };
  ...
  {addresses.map((addr) => (
    <li key={addr.id} className=\"mb-2 p-3 border rounded flex items-center justify-between\">
      ...
      <div className=\"flex items-center gap-2\">
        ...
        {addr.id !== defaultAddressId && (
          <button onClick={() => setAsDefault(addr.id)} className=\"text-xs px-3 py-1 rounded bg-indigo-600 text-white hover:bg-indigo-700\">Predeterminada</button>
        )}
        ...
        <div className="mt-6">
          ...
          <form onSubmit={handleGuestAddressSubmit}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <input placeholder="Calle" value={data.street} onChange={(e) => setData('street', e.target.value)} className={`w-full border p-2 rounded ${errors.street ? 'border-red-400' : 'border-gray-300'}`} aria-invalid={Boolean(errors.street)} aria-describedby={errors.street ? 'street-error' : undefined} />
                {errors.street && <p id="street-error" className="mt-1 text-red-500 text-sm">{errors.street}</p>}
              </div>
              <div>
                <input placeholder="Ciudad" value={data.city} onChange={(e) => setData('city', e.target.value)} className={`w-full border p-2 rounded ${errors.city ? 'border-red-400' : 'border-gray-300'}`} aria-invalid={Boolean(errors.city)} aria-describedby={errors.city ? 'city-error' : undefined} />
                {errors.city && <p id="city-error" className="mt-1 text-red-500 text-sm">{errors.city}</p>}
              </div>
              <div>
                <input placeholder="Provincia" value={data.province} onChange={(e) => setData('province', e.target.value)} className={`w-full border p-2 rounded ${errors.province ? 'border-red-400' : 'border-gray-300'}`} aria-invalid={Boolean(errors.province)} aria-describedby={errors.province ? 'province-error' : undefined} />
                {errors.province && <p id="province-error" className="mt-1 text-red-500 text-sm">{errors.province}</p>}
              </div>
              <div>
                <input placeholder="Código Postal" value={data.zip_code} onChange={(e) => setData('zip_code', e.target.value)} className={`w-full border p-2 rounded ${errors.zip_code ? 'border-red-400' : 'border-gray-300'}`} aria-invalid={Boolean(errors.zip_code)} aria-describedby={errors.zip_code ? 'zip-error' : undefined} />
                {errors.zip_code && <p id="zip-error" className="mt-1 text-red-500 text-sm">{errors.zip_code}</p>}
              </div>
              <div>
                <input placeholder="País" value={data.country} onChange={(e) => setData('country', e.target.value)} className={`w-full border p-2 rounded ${errors.country ? 'border-red-400' : 'border-gray-300'}`} aria-invalid={Boolean(errors.country)} aria-describedby={errors.country ? 'country-error' : undefined} />
      ...
      {showAddressModal && (
        <AddressModal closeModal={() => setShowAddressModal(false)} onAddressSaved={handleAddressAdded} />
      )}
    </div>
  );
}
